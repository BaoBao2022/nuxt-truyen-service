...
export default function wbController() {
    ...
    const readComic = async (req: Request, res: Response) => {
        ...
        const response = await axios.get(`/${slug}/${_id}.json`, {
            baseURL: 'https://weeboo.vn/_next/data/4Gs69jVc_-x-BI8xSHAfe/truyen-tranh-chapter',
            params: {
                slug: slug,
            }
        })
    ...
    const getChapters = async (req: Request, res: Response) => {
        ...
        Chapter.find({
            ...
            // documents is all the items that already exists
            let newItems = chapters.filter((item: any) => !documents.find((doc) => doc.slug === item.slug));
            newItems = newItems.map((chap: any) => ({...chap, comic_id: comic_id, comic_slug: comic_slug}))
